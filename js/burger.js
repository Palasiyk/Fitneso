...
function mobileMenu_build(){
    let menu_content = document.createElement('div');                     //stvorennia novogo elementa
...
function mobileMenu_toggle(){
    document.getElementById('mobile-menu').classList.toggle('open');  //toggle-dodatu,perevirutu
...
function mobileMenu_destroy(){
    ...
    if (mobileMenu != null){
        mobileMenu.remove();
        document.getElementById(`mobile-menu-content`).remove(); //remove-vudalutu
    }
}
